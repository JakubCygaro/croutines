...
void cco_send(cco_Coroutine* self, cco_Ctx_p ctx, cco_Co_handle co, char msg[128]);
void cco_recv(cco_Coroutine* self, cco_Ctx_p ctx, char out[128]);
...
static void cco_recv_impl(cco_Coroutine* self, cco_Process* proc, char out[128])
{
    proc->recv_buf = out;
...
static void cco_deliver_messages(cco_MessageQueue* mq, cco_Process** procs, int procs_sz)
{
    cco_Message* delay_stack[128] = { };
    int dp = -1;
    while (mq->head) {
...
cco_Sched cco_Sched_new()
{
    return (cco_Sched) {
        .mq = (cco_MessageQueue) { },
        .procs = (cco_Process**)calloc(CCO_SCHED_INIT_PROC_COUNT, sizeof(cco_Process*)),
        .proc_count = 0,
        .proc_cap = CCO_SCHED_INIT_PROC_COUNT,
        .pqueue = (cco_ProcQueue) { },
    };
}
cco_Co_handle cco_Sched_add_coroutine(cco_Sched* sched, cco_Coroutine co)
{
